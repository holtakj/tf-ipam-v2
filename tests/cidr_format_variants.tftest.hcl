# CIDR format coverage tests for base_network_cidr and reservations.
# Includes valid IPv4 examples and invalid formats, including IPv6 variants.

run "valid_ipv4_base_format_10_0_0_0_24" {
  command = plan

  variables {
    base_network_cidr = "10.0.0.0/24"
    min_prefix_length = 24
    max_prefix_length = 24
  }

  assert {
    condition     = output.subnet_count_by_cidr_size["/24"] == 1
    error_message = "A standard IPv4 CIDR must be accepted."
  }
}

run "valid_ipv4_base_format_172_16_0_0_20" {
  command = plan

  variables {
    base_network_cidr = "172.16.0.0/20"
    min_prefix_length = 20
    max_prefix_length = 20
  }

  assert {
    condition     = output.subnet_count_by_cidr_size["/20"] == 1
    error_message = "Another standard IPv4 CIDR must be accepted."
  }
}

run "invalid_base_non_canonical_fails" {
  command = plan

  variables {
    base_network_cidr = "10.0.0.1/24"
    min_prefix_length = 24
    max_prefix_length = 24
  }

  expect_failures = [
    var.base_network_cidr,
  ]
}

run "invalid_base_missing_prefix_fails" {
  command = plan

  variables {
    base_network_cidr = "10.0.0.0"
    min_prefix_length = 24
    max_prefix_length = 24
  }

  expect_failures = [
    var.base_network_cidr,
  ]
}

run "invalid_base_prefix_33_fails" {
  command = plan

  variables {
    base_network_cidr = "10.0.0.0/33"
    min_prefix_length = 24
    max_prefix_length = 24
  }

  expect_failures = [
    var.base_network_cidr,
  ]
}

run "invalid_base_octet_out_of_range_fails" {
  command = plan

  variables {
    base_network_cidr = "300.0.0.0/24"
    min_prefix_length = 24
    max_prefix_length = 24
  }

  expect_failures = [
    var.base_network_cidr,
  ]
}

run "invalid_base_ipv6_compressed_fails" {
  command = plan

  variables {
    base_network_cidr = "2001:db8::/32"
    min_prefix_length = 24
    max_prefix_length = 24
  }

  expect_failures = [
    var.base_network_cidr,
  ]
}

run "invalid_base_ipv6_expanded_fails" {
  command = plan

  variables {
    base_network_cidr = "2001:0db8:0000:0000:0000:ff00:0042:8329/64"
    min_prefix_length = 24
    max_prefix_length = 24
  }

  expect_failures = [
    var.base_network_cidr,
  ]
}

run "invalid_base_ipv4_mapped_ipv6_fails" {
  command = plan

  variables {
    base_network_cidr = "::ffff:192.0.2.128/128"
    min_prefix_length = 24
    max_prefix_length = 24
  }

  expect_failures = [
    var.base_network_cidr,
  ]
}

run "invalid_reservation_missing_prefix_fails" {
  command = plan

  variables {
    base_network_cidr = "10.0.0.0/24"
    min_prefix_length = 24
    max_prefix_length = 24

    reservations = {
      bad = "10.0.0.10"
    }
  }

  expect_failures = [
    var.reservations,
  ]
}

run "invalid_reservation_empty_name_fails" {
  command = plan

  variables {
    base_network_cidr = "10.0.0.0/24"
    min_prefix_length = 24
    max_prefix_length = 24

    reservations = {
      "" = "10.0.0.0/24"
    }
  }

  expect_failures = [
    var.reservations,
  ]
}

run "invalid_reservation_ipv6_fails" {
  command = plan

  variables {
    base_network_cidr = "10.0.0.0/24"
    min_prefix_length = 24
    max_prefix_length = 24

    reservations = {
      bad = "2001:db8::/64"
    }
  }

  expect_failures = [
    var.reservations,
  ]
}

run "invalid_reservation_non_canonical_fails" {
  command = plan

  variables {
    base_network_cidr = "10.0.0.0/24"
    min_prefix_length = 24
    max_prefix_length = 24

    reservations = {
      bad = "10.0.0.1/30"
    }
  }

  expect_failures = [
    var.reservations,
  ]
}
