...
variable "reserved" {
  description = "Stable reservation map keyed by reservation name. Reservation names must be unique. Values may be a canonical IPv4 CIDR (e.g. 10.0.0.0/24), an IPv4 address (treated as /32), or an IP range in start-end notation (e.g. 10.0.1.0-10.0.1.255)."
  type        = map(string)
  default     = {}
  ...
  validation {
    condition = alltrue([
      ...
      length(reservation_name) > 0 && (
        (
          ...
          can(regex("^([0-9]{1,3}\\.){3}[0-9]{1,3}$", reservation_value)) &&
          can(cidrhost(format("%s/32", reservation_value), 0))
          ) || (
    ...
    error_message = "Each reserved entry must have a non-empty key and a value that is either a canonical IPv4 CIDR (e.g. 10.0.0.0/24, host bits must be zero), an IPv4 address (treated as /32), or an IP range (e.g. 10.0.1.0-10.0.1.255). IPv6 is not supported."
  }
}
